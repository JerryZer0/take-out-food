const {bestCharge, turnIntoIdAndCount, completeOrder, calculateSubtotal,amountInPromotion1, amountInPromotion2, generateReceipt, findBestPromotion} = require('../src/best-charge');
...
describe('计算价格方式一', ()=> {
  ...
  it('should calculate the first promotion', ()=> {
    expect(JSON.stringify(summary)).toEqual(expected);
  })
})
...
describe('计算价格方式二', ()=> {
  ...
  it('should calculate the second promotion', ()=> {
    expect(JSON.stringify(summary)).toEqual(expected);
  })
})
...
describe('选择方式', ()=> {
  ...
  const expected = [{"itemsList":[{"id":"ITEM0001","name":"黄焖鸡","price":18,"count":"1","subtotal":18},{"id":"ITEM0013","name":"肉夹馍","price":6,"count":"2","subtotal":12},{"id":"ITEM0022","name":"凉皮","price":8,"count":"1","subtotal":8}]},{"type":"指定菜品半价(黄焖鸡,凉皮)"},{"reduce":13},{"amount":25}];
  const summary = findBestPromotion(receiptObject1,receiptObject2);
  it('should find the best promotion', ()=> {
...
describe('生成清单', ()=> {
  ...
  it('should generateReceipt the receipt', ()=> {
    expect(summary).toEqual(expected);
  })
})
